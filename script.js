...
if (typeof Chart === 'undefined') {
  document.querySelectorAll('.chart-wrap, .kpi-row').forEach(function(el){
    el.innerHTML = '<div style="font-family:\'IBM Plex Mono\',monospace;font-size:12.5px;color:#E5484D;padding:16px;border:1px solid #E5484D;">⚠ Plik chart.min.js nie załadował się. Upewnij się, że leży w tym samym folderze co index.html i script.js, i odśwież stronę.</div>';
  });
...
/* ============ DATA GENERATION ============ */
const CLIENTS = [
  {name:'Projekt A — design dziecięcy', seed: 11, meanOutflow:{rotating:34, nonrotating:6}, var:{rotating:9, nonrotating:3}, restock:{rotating:9, nonrotating:20}, restockQty:{rotating:340, nonrotating:90}, start:{rotating:1250, nonrotating:420}, leadTimeDays:5},
  {name:'Projekt B — sprzęt sportowy', seed: 23, meanOutflow:{rotating:22, nonrotating:4}, var:{rotating:6, nonrotating:2}, restock:{rotating:11, nonrotating:24}, restockQty:{rotating:260, nonrotating:70}, start:{rotating:980, nonrotating:360}, leadTimeDays:7},
  {name:'Projekt C — oświetlenie', seed: 37, meanOutflow:{rotating:27, nonrotating:5}, var:{rotating:7, nonrotating:2.5}, restock:{rotating:10, nonrotating:22}, restockQty:{rotating:300, nonrotating:80}, start:{rotating:1100, nonrotating:390}, leadTimeDays:10},
  {name:'Projekt D — kosmetyki/biżuteria', seed: 41, meanOutflow:{rotating:31, nonrotating:5.5}, var:{rotating:13, nonrotating:4}, restock:{rotating:8, nonrotating:18}, restockQty:{rotating:280, nonrotating:75}, start:{rotating:1050, nonrotating:340}, leadTimeDays:6},
];

function genSeries(seed, days, meanOutflow, outflowVar, restockEvery, restockQty, startStock){
  const rand = mulberry32(seed);
  ...
  for(let d=0; d<days; d++){
    ...
    stock -= outflow;
    if(d>0 && d % restockEvery === 0){ stock += restockQty * (0.8 + rand()*0.4); }
...
function forecastFrom(series, trainWindow, horizon){
  ...
  const xs = [], ys = [];
  for(let i=startIdx;i<n;i++){ xs.push(i); ys.push(series[i]); }
...
function backtestMAPE(series, trainWindow, testWindow){
  ...
  const xs=[], ys=[];
  for(let i=startIdx;i<trainEnd;i++){ xs.push(i); ys.push(series[i]); }
  const {slope, intercept} = linreg(xs, ys);
  let errSum = 0, count=0;
  for(let i=trainEnd;i<n;i++){
    const pred = Math.max(0, slope*i + intercept);
    const actual = series[i];
    if(actual > 0){ errSum += Math.abs(actual-pred)/actual; count++; }
  }
  return count ? (errSum/count*100) : null;
}

let invChart, paretoChart, leadChart;
let currentInvExport = null, currentAbcExport = null, currentLeadExport = null;
...
function renderInventoryDash(){
  ...
  const days = 60, horizon = 14;
  const series = genSeries(c.seed + (catKey==='rotating'?0:100), days, c.meanOutflow[catKey], c.var[catKey], c.restock[catKey], c.restockQty[catKey], c.start[catKey]);
  ...
  const avgOutflow = deltas.length ? deltas.reduce((a,b)=>a+b,0)/deltas.length : c.meanOutflow[catKey];
  ...
  for(let i=0;i<forecast.length;i++){
    if(forecast[i] <= reorderPoint){ reorderDay = i; break; }
  }
  ...
  document.getElementById('chartSubtitle').textContent = `${c.name} · ${category==='rotating'?'rotujące':'nierotujące'}`;
  ...
  const exportRows = [['Dzień','Typ','Stan (szt.)','Punkt zamówienia','Zapas bezpieczeństwa','Projekt','Kategoria']];
  for(let i=0;i<days;i++){
    exportRows.push([i-days+1, 'Historia', series[i], reorderPoint, safetyStock, c.name, category==='rotating'?'rotujące':'nierotujące']);
  }
  for(let h=1;h<=horizon;h++){
    exportRows.push([h, 'Prognoza', Math.round(forecast[h]), reorderPoint, safetyStock, c.name, category==='rotating'?'rotujące':'nierotujące']);
  }
  currentInvExport = {filename:`stany-magazynowe_${c.name.replace(/[^a-z0-9]+/gi,'-')}_${category}.csv`, rows: exportRows};

  // build chart datasets
  const labels = [];
  for(let i=0;i<days+horizon;i++) labels.push('D'+(i-days+1<=0? i - (days-1) : i));
  const histData = series.map(v=>v).concat(Array(horizon).fill(null));
  const foreData = Array(days-1).fill(null).concat(forecast);
  ...
  invChart = new Chart(ctx, {
    ...
    data:{
      labels,
      datasets:[
        {label:'Stan historyczny', data:histData, borderColor:'#2DD4BF', backgroundColor:'transparent', borderWidth:2, pointRadius:0, tension:.15},
        {label:'Prognoza', data:foreData, borderColor:'#FF6A13', backgroundColor:'transparent', borderWidth:2, borderDash:[6,4], pointRadius:0, tension:.15},
        {label:'Punkt zamówienia', data:reorderLine, borderColor:'#E5484D', borderWidth:1, pointRadius:0, borderDash:[2,3]},
        {label:'Zapas bezpieczeństwa', data:safetyLine, borderColor:'#8A93A3', borderWidth:1, pointRadius:0, borderDash:[1,3]},
      ]
    },
    options:{
      ...
      scales:{
        ...
        y:{grid:{color:'#2E3540'}, title:{display:true,text:'szt.',color:'#8A93A3'}}
      }
    }
  });
}
document.getElementById('clientSelect').addEventListener('change', renderInventoryDash);
document.getElementById('categorySelect').addEventListener('change', renderInventoryDash);
...
function renderABCDash(){
  ...
  let cum = 0;
  skus.forEach(s=>{ cum += s.value; s.cumShare = cum/totalValue; });
  skus.forEach(s=>{
  ...
  const matrixCounts = {};
  ['A','B','C'].forEach(a=>['X','Y','Z'].forEach(x=>matrixCounts[a+x]=0));
  ...
  ['A','B','C'].forEach(a=>{
    ['X','Y','Z'].forEach(x=>{
      ...
      const color = isRotating ? 'var(--accent-2)' : (isDead ? 'var(--danger)' : 'var(--text)');
  ...
  const rotatingCount = skus.filter(s=>(s.abc==='A'||s.abc==='B') && (s.xyz==='X'||s.xyz==='Y')).length;
  ...
  const exportRows = [['Lp','Wartość (jedn. umowne)','Udział skumulowany (%)','Klasa ABC','Klasa XYZ']];
  skus.forEach((s,i)=>{
    exportRows.push([i+1, s.value.toFixed(1), (s.cumShare*100).toFixed(1), s.abc, s.xyz]);
  });
  currentAbcExport = {filename:'rotacja-abc-xyz_wszystkie-sku.csv', rows: exportRows};
  ...
  paretoChart = new Chart(ctx, {
    ...
    data:{
      labels,
      datasets:[
        {type:'bar', label:'Udział wartości', data:skus.map(s=>s.value), backgroundColor:'#2DD4BF66', borderWidth:0, yAxisID:'y'},
        {type:'line', label:'Skumulowany %', data:skus.map(s=>s.cumShare*100), borderColor:'#FF6A13', pointRadius:0, borderWidth:2, yAxisID:'y1'},
      ]
    },
    options:{
      responsive:true, maintainAspectRatio:false,
      ...
      scales:{
        x:{display:false},
        y:{grid:{color:'#2E3540'}, title:{display:true,text:'wartość (jedn. umowne)',color:'#8A93A3'}},
        y1:{position:'right', min:0, max:100, grid:{display:false}, title:{display:true,text:'% skumulowany',color:'#8A93A3'}},
      }
    }
  });
}
...
function renderLeadDash(){
  ...
  let leadRows = '';
  const exportRows = [['Projekt','Czas przyjęcia (godz.)','Czas wydania (godz.)','Łącznie (godz.)']];
  CLIENTS.forEach((c,i)=>{
    ...
    exportRows.push([c.name, receiving[i].toFixed(1), shipping[i].toFixed(1), total.toFixed(1)]);
  });
  leadTbody.innerHTML = leadRows;
  currentLeadExport = {filename:'czasy-realizacji_wg-projektu.csv', rows: exportRows};
  ...
  leadChart = new Chart(ctx, {
    ...
    options:{
      ...
      scales:{
        ...
        y:{grid:{color:'#2E3540'}, title:{display:true,text:'godziny',color:'#8A93A3'}}
      }
    }
  });
}
...
document.getElementById('exportInvBtn').addEventListener('click', ()=>{
  if(currentInvExport) downloadCSV(currentInvExport.filename, currentInvExport.rows);
});
document.getElementById('exportAbcBtn').addEventListener('click', ()=>{
  if(currentAbcExport) downloadCSV(currentAbcExport.filename, currentAbcExport.rows);
});
document.getElementById('exportLeadBtn').addEventListener('click', ()=>{
  if(currentLeadExport) downloadCSV(currentLeadExport.filename, currentLeadExport.rows);
});
